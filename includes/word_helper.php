...
function rjpes_pdf_merge($cover_pdf_path, $body_pdf_path, $output_pdf_path, $journal_number = '', $volume = '', $issue = '', $month_year = '') {
    ...
    $py_content .= "\n";
    $py_content .= "def has_cid_fonts(page):\n";
    $py_content .= "    \"\"\"Detect Identity-H CID fonts that Chrome PDFium cannot render.\"\"\"\n";
    $py_content .= "    for f in page.get_fonts():\n";
    $py_content .= "        enc = f[5] if len(f) > 5 else ''\n";
    $py_content .= "        if enc == 'Identity-H':\n";
    $py_content .= "            return True\n";
    $py_content .= "    return False\n";
    $py_content .= "\n";
    $py_content .= "try:\n";
    $py_content .= "    # 1. Open cover PDF and insert body PDF pages using fitz\n";
    $py_content .= "    #    fitz insert_pdf() preserves all embedded TrueType/OpenType fonts intact\n";
    ...
    $py_content .= "    \n";
    $py_content .= "    # 3. Fix browser font rendering: body pages that use Identity-H CID fonts\n";
    $py_content .= "    #    (generated by MS Word) cannot be rendered by Chrome/Firefox's built-in\n";
    $py_content .= "    #    PDF viewer. Re-render those pages as high-quality grayscale JPEG images\n";
    $py_content .= "    #    so any browser can display them correctly.\n";
    $py_content .= "    out = fitz.open()\n";
    $py_content .= "    # Cover page: always keep as-is (uses Helvetica Type1, renders everywhere)\n";
    $py_content .= "    out.insert_pdf(doc, from_page=0, to_page=0)\n";
    $py_content .= "    for i in range(1, len(doc)):\n";
    $py_content .= "        page = doc[i]\n";
    $py_content .= "        w = page.rect.width\n";
    $py_content .= "        h = page.rect.height\n";
    $py_content .= "        if has_cid_fonts(page):\n";
    $py_content .= "            # Render to grayscale JPEG at 150 DPI — crisp text, small file\n";
    ...
    $py_content .= "        else:\n";
    $py_content .= "            out.insert_pdf(doc, from_page=i, to_page=i)\n";
